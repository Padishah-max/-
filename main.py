...
print('telegram package path:', getattr(telegram, '__file__', '?'), flush=True)
print('python-telegram-bot version reported:', getattr(telegram, '__version__', '?'), flush=True)
ver = tuple(int(x) for x in getattr(telegram, '__version__', '0.0').split('.')[:2])
if ver < (20, 0):
    raise RuntimeError('Wrong PTB version loaded: %s. Need >= 20.0' % getattr(telegram, '__version__', '?'))
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if update.effective_user:
        ...
        with db() as conn:
            conn.execute('INSERT INTO users(user_id, username, first_name, last_name) VALUES(?,?,?,?) ON CONFLICT(user_id) DO UPDATE SET username=excluded.username, first_name=excluded.first_name, last_name=excluded.last_name',(u.id, u.username, u.first_name, u.last_name))
...
async def begin(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not is_admin(update.effective_user.id):
        await update.message.reply_text('Только администратор может начинать.'); return
    state = await ensure_state(update.effective_chat.id)
    state.index = 0; state.last_poll_message_id = None
...
async def next_q(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not is_admin(update.effective_user.id):
        await update.message.reply_text('Только администратор может отправлять вопросы.'); return
...
async def close_poll(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not is_admin(update.effective_user.id):
        await update.message.reply_text('Только администратор может закрывать опрос.'); return
    ...
    state.index += 1; state.last_poll_message_id = None
    if state.index < len(QUESTIONS):
...
async def on_button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data.startswith('set_country:'):
        ...
        with db() as conn:
            conn.execute('INSERT INTO users(user_id, country) VALUES(?,?) ON CONFLICT(user_id) DO UPDATE SET country=excluded.country',(cq.from_user.id, country))
        await cq.answer('Страна сохранена'); await cq.edit_message_text(f'Вы выбрали: {country}')
