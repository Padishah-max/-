...
def validate_questions(data: List[dict]) -> List[Question]:
    ...
    for i, q in enumerate(data, start=1):
        ...
        qs.append(Question(text, options, correct, multiple))
    return qs
...
async def export_results_file() -> str:
    ...
    for c,s in stats.items():
        tot=s["tot"]; ok=s["ok"]; bad=tot-ok; ppl=len(users_by_country.get(c,set()))
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "/restart — пройти заново (сброс прошлой попытки)\n\n"
        "Администратор:\n"
        "/report — Excel-отчёт\n"
...
async def cmd_setq(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        return
    if not context.args:
        await update.message.reply_text("Укажи RAW-ссылку на JSON: /setq https://raw.githubusercontent.com/.../questions.json")
        return
    url = context.args[0]
    try:
